...
class Node:
  ...
  def send_next_message(self):
    ...
    # If there's an ACK pending, piggyback it
    if self.ack_pending:
        print('{}Piggybacking ACK on DATA (ack_seq={})'.format(self.printspaces, self.pending_ack_seq))
        self.ack_pending = False
        self.pending_ack_seq = None
        if self.ack_timer:
    ...
    self.lastmsg = msg
    self.transmit_frame(msg, FrameType.DLL_DATA, self.nextframetosend)
    self.nextframetosend = 1 - self.nextframetosend
    disable_application()
  ...
  def physical_ready(self, linkno: int, framebytes: bytes):
    ...
    #     FrameType.DLL_ACK, f.seq)
      ...
      if self.ack_timer:
        stop_timer(self.ack_timer)
      self.ack_timer = start_timer(Event.TIMER2, 1000000)  # 1 second
  ...
  def piggyback_timeout(self):
    if self.ack_pending:
        ...
        self.transmit_frame(bytes(), FrameType.DLL_ACK, self.pending_ack_seq)
        self.ack_pending = False
        self.pending_ack_seq = None
        self.ack_timer = None
  ...
  def reboot_node(self):
    ...
    set_handler(Event.TIMER2, self.piggyback_timeout)
