...
class Simulator:
  ...
  def generate_application_message(self, sender):
    ...
    if dests:
      ...
      messagesize = random.randint(defs.MIN_MESSAGE_SIZE, defs.MAX_MESSAGE_SIZE)
      messagebytes = secrets.token_bytes(messagesize)
        ...
        for link in host['links']:
          ...
          if 'to' in link:
            if link['to'] in hostlookup:
              linkid = '-'.join(sorted([host['name'], link['to']]))
