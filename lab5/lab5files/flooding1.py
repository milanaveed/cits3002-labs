...
# This is an implementation of a very naive flooding algorithm.
# Whenever a new Network Layer packet requires delivery, it is transmitted on
# *all* physical links (except loopback). To limit the combinatoric explosion
# in the number of data packets in the whole network, data packets are discarded
# after they have travelled MAXHOPS hops.
#
# The purpose of this example is to demonstrate the flooding process itself, and
# for this reason only a minimal datalink layer protocol is required to deliver
# the packets.
#
# This naive flooding algorithm exhibits a very poor efficiency.
# Over the 8 nodes in the FLOODING1 file, the efficiency is typically
# only about 1.5%
...
class Node:
  ...
  def flood1(self, packetbytes: bytes):
    """flood1 is a very basic routing strategy which transmits the outgoing
    packet on every available link."""
