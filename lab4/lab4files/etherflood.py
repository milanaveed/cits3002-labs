...
class Node:
  ...
  def schedule_send_on_clear(self, time=BACKOFF_SLOT):
    ...
    self.send_on_clear_timer = start_timer(Event.TIMER3, time)
  ...
  def collision(self):
    if self.frame and self.finished_sending_timer:
      ...
      # todo: replace this with binary exponential backoff based on number of
      # consecutive collisions of this frame.
      self.frame = None
      start_timer(Event.TIMER1, random.randrange(self.freq) + 1)
  ...
  def reboot_node(self, args):
    ...
    set_handler(Event.TIMER3, self.send_on_clear)
